...
const ChatWidget = ({
    ...
    const { primaryColor, description, region, apiGateway, contactFlowId, instanceId, agentDisplayName } = useAppConfig();
    ...
    const successHandler = (chatSession) => {
        ...
        chatSession.incomingItemDecorator = function (item) {
            ...
            if (item.displayName.indexOf(':') !== -1) {
              const splitStr = item.displayName.split(':')
              if (splitStr.length == 2 && splitStr[0].length > 10 && splitStr[0].length < 30) {
                // seems to be an amazon user long id, change it to agent display name
                item.displayName = agentDisplayName;
              }
            }
            return item;
        }
